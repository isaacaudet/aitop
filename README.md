...
You're using AI coding tools every day. Do you know what they cost?
...
**clawdtop reads data already on your machine** — no API keys, no cloud, no telemetry. Just `go install` and see the truth.
...
> **Note:** Requires `CGO_ENABLED=1` for Cursor's SQLite database. If you don't use Cursor, set `CGO_ENABLED=0` for a simpler build.
...
clawdtop uses current API pricing to calculate costs:
